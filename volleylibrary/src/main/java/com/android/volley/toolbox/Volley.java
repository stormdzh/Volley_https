...
public class Volley {
	...
	public static RequestQueue newRequestQueue(Context context) {
		...
		return newRequestQueue(context, null, false, -1);
	}
	public static RequestQueue newRequestQueue(Context context,int certId) {
		// 如果你目前还没有证书,那么先用下面的这行代码,http可以照常使用.
		// return newRequestQueue(context, null, false, 0);
		// 此处R.raw.certificateName 表示你的证书文件,替换为自己证书文件名字就好
		//return newRequestQueue(context, null, true, R.raw.certificateName);
		if(certId==-1)
			...
			return newRequestQueue(context, null, true, certId);
	}
